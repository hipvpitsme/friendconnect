...
public class DefaultServers {
    ...
    public static FormWindowSimple createFormWindow(String uuid) {
        removeUser(uuid);
        FormWindowSimple window = new FormWindowSimple("Default Servers", "");
        try {
            ResultSet rs = Main.c.createStatement().executeQuery("select * from players.defaultservers");
            ...
            return ConfirmationScreen.createFormWindow(uuid, "An Error Occurred!", "An error occurred while trying to load the default servers!", MainWindow.createFormWindow(uuid));
        }
